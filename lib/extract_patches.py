...
def load_data(data_path_list_file):
    ...
    for i in range(len(img_list)):
        ...
        imgs = (
            ...
            if imgs is None
            else np.concatenate((imgs, np.expand_dims(img, 0)))
        )
        groundTruth = (
            np.expand_dims(gt, 0)
            if groundTruth is None
            else np.concatenate((groundTruth, np.expand_dims(gt, 0)))
        )
...
def get_data_train(data_path_list, patch_height, patch_width, N_patches, inside_FOV):
    ...
    print(
        "\nTrain images shape: {}, vaule range ({} - {}):".format(
            train_imgs.shape, str(np.min(train_imgs)), str(np.max(train_imgs))
        )
    )
    ...
    print(
        "train patches shape: {}, value range ({} - {})".format(
...
def extract_random(
    full_imgs, full_masks, full_FOVs, patch_h, patch_w, N_patches, inside="not"
):
    ...
    print(
        "patches per image: " + str(patch_per_img),
    ...
    for i in range(full_imgs.shape[0]):  # loop over the all images
        ...
        while k < patch_per_img:
            ...
            if inside == "center" or inside == "all":
                if not is_patch_inside_FOV(
                    x_center, y_center, full_FOVs[i, 0], patch_h, patch_w, mode=inside
                ):
                    continue
            patch = full_imgs[
                i,
...
def is_patch_inside_FOV(x, y, fov_img, patch_h, patch_w, mode="center"):
    ...
    if mode == "center":
        ...
        fov_patch = fov_img[
            y - int(patch_h / 2) : y + int(patch_h / 2),
...
def get_data_test_overlap(
    test_data_path_list, patch_height, patch_width, stride_height, stride_width
):
    ...
    print(
        "\nTest images shape: {}, vaule range ({} - {}):".format(
            test_imgs.shape, str(np.min(test_imgs)), str(np.max(test_imgs))
        )
    )
    ...
    print(
        "test patches shape: {}, value range ({} - {})".format(
...
def paint_border_overlap(full_imgs, patch_h, patch_w, stride_h, stride_w):
    ...
    assert (
        full_imgs.shape[1] == 1 or full_imgs.shape[1] == 3
    ...
    if leftover_h != 0:  # change dimension of img_h
        print(
            "\nthe side H is not compatible with the selected stride of "
            + str(stride_h)
        )
        # print("img_h " +str(img_h) + ", patch_h " +str(patch_h) + ", stride_h " +str(stride_h))
        print("(img_h - patch_h) MOD stride_h: " + str(leftover_h))
        print(
            "So the H dim will be padded with additional "
            + str(stride_h - leftover_h)
            + " pixels"
        )
        ...
        tmp_full_imgs[
            0 : full_imgs.shape[0], 0 : full_imgs.shape[1], 0:img_h, 0:img_w
        ] = full_imgs
    ...
    if leftover_w != 0:  # change dimension of img_w
        ...
        print(
            "So the W dim will be padded with additional "
            + str(stride_w - leftover_w)
            + " pixels"
        )
        tmp_full_imgs = np.zeros(
            (
        ...
        for h in range((img_h - patch_h) // stride_h + 1):
            for w in range((img_w - patch_w) // stride_w + 1):
                ...
                full_sum[
                    ...
                    h * stride_h : (h * stride_h) + patch_h,
                    w * stride_w : (w * stride_w) + patch_w,
                ] += 1  # Accumulate the number of predictions
                k += 1

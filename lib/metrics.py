...
class Evaluate:
    ...
    def add_batch(self, batch_tar, batch_out):
        ...
        self.target = (
            ...
            else np.concatenate((self.target, batch_tar))
        )
        self.target = self.target == 1
        self.output = (
            batch_out
            if self.output is None
            else np.concatenate((self.output, batch_out))
        )
    ...
    def auc_roc(self, plot=False):
        ...
        if plot and self.save_path is not None:
            ...
            plt.figure()
            plt.plot(
                fpr, tpr, "-", label="Area Under the Curve (AUC = %0.4f)" % AUC_ROC
            )
            plt.title("ROC curve")
    ...
    def auc_pr(self, plot=False):
        ...
        if plot and self.save_path is not None:
            ...
            plt.plot(
                ...
                precision,
                "-",
                label="Area Under the Curve (AUC = %0.4f)" % AUC_pr,
            )
    ...
    def f1_score(self):
        ...
        F1_score = f1_score(
            self.target, pred, labels=None, average="binary", sample_weight=None
        )

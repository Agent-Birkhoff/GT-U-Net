...
class Test:
    def __init__(self, args):
        self.args = args
        assert (
            args.stride_height <= args.test_patch_height
            and args.stride_width <= args.test_patch_width
        )
        # save path
        self.path_experiment = join(args.outf, args.save)

        (
            self.patches_imgs_test,
            self.test_imgs,
            self.test_masks,
            self.test_FOVs,
            self.new_height,
    ...
    def evaluate(self):
        self.pred_imgs = recompone_overlap(
            ...
            self.new_height,
            self.new_width,
            self.args.stride_height,
            self.args.stride_width,
        )
        ...
        np.save(
            "{}/result.npy".format(self.path_experiment), np.asarray([y_true, y_scores])
        )

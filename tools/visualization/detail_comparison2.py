...
def crop_and_resize(img, center, crop_length, target_shape, inter=None):
    crop_img = img[
        center[1] - crop_length // 2 : center[1] + crop_length // 2,
...
if __name__ == "__main__":
    result = readImg(
        "/ssd/lzq/sf3/output/s_3/result_img/Original_GroundTruth_Prediction2.png"
    )
    ...
    prob_block = crop_and_resize(
        result_list[1], (w0, h0), block, (ori_img.shape[0], ori_img.shape[0]), inter=2
    )
    bin_block = crop_and_resize(
        result_list[2], (w0, h0), block, (ori_img.shape[0], ori_img.shape[0]), inter=1
    )
    gt_block = crop_and_resize(
        result_list[3], (w0, h0), block, (ori_img.shape[0], ori_img.shape[0]), inter=1
    )
    ...
    ori_block = crop_and_resize(
        ori_img, (w0, h0), block, (ori_img.shape[0], ori_img.shape[0]), inter=2
    )
    ...
    cv2.line(
        ori_img, (w0 - block // 2, h0 + block // 2), ori_img.shape[::-1][1:], 255, 5
    )

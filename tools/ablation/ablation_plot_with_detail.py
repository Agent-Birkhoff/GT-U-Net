...
result_list = {
    "d_base": "./experiments/db1_new/result.npy",
...
mark_inset(
    ax, axins, loc1=3, loc2=1, fc="none", ec="k", lw=1
)  # 建立连线 loc1和loc2取值为1，2，3，4，对应右上，左上，左下，右下
...
mark_inset(
    ax, axins, loc1=1, loc2=4, fc="none", ec="k", lw=1
)  # 建立连线 loc1和loc2取值为1，2，3，4，对应右上，左上，左下，右下
for name, path in result_list.items():
    data = np.load(path[0])
    target, output = data
    ...
    ax.plot(
        recall, precision, linestyle=path[1], label=name + " (AUC = %0.4f)" % AUC_pr
    )
    axins.plot(
        recall, precision, linestyle=path[1], label=name + " (AUC = %0.4f)" % AUC_pr
    )
